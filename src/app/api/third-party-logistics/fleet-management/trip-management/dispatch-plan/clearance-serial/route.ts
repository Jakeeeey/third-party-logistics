...
export async function GET(request: Request) {
    ...
    try {
        ...
        const divisionBranchesRes = await fetcher(`/branches?filter[division_id][_eq]=1&fields=id&limit=-1`);
        const validBranchIds = (divisionBranchesRes.data || []).map((b: { id: number }) => b.id);
        ...
        const [transSerialsRes] = await Promise.all([
            transDetailIds.length > 0 ? fetcher(`/unfulfilled_sales_transaction_serial?filter[unfulfilled_sales_transaction_detail_id][_in]=${transDetailIds.join(',')}&limit=-1`) : Promise.resolve({ data: [] }),
        ]);
        const transSerials = transSerialsRes?.data || [];
                    ...
                    return {
                        ...
                        scannedSerials: (() => {
                            ...
                            const details = transDetails.filter((d: { unfulfilled_sales_transaction_id: number; id: number }) => {
                                ...
                                // Also ensure there are Serials for this detail
                                return matched && transSerials.some((r: { unfulfilled_sales_transaction_detail_id: number }) => r.unfulfilled_sales_transaction_detail_id === d.id);
                            });
                            const map: Record<string | number, string[]> = {};
                            details.forEach((d: { id: number; sales_invoice_detail_id: number }) => {
                                const matchedSerials = transSerials
                                    .filter((r: { unfulfilled_sales_transaction_detail_id: number; serial_number: string }) => r.unfulfilled_sales_transaction_detail_id === d.id)
                                    .map((r: { serial_number: string }) => r.serial_number);
                                if (matchedSerials.length > 0) {
                                    map[d.sales_invoice_detail_id] = matchedSerials;
                                }
                            });
                            return map;
                        })()
            ...
            if (inv.status !== 'Fulfilled' && inv.status !== 'Fulfilled with Returns') {
                ...
                if (detailIdsToLog.size > 0) {
                    detailIdsToLog.forEach((detailIdStr) => {
                        ...
                        const wasInteracted = Object.prototype.hasOwnProperty.call(inv.scannedSerials || {}, detailIdStr) || Object.prototype.hasOwnProperty.call(inv.missingQtys || {}, detailIdStr);
                ...
                if (existingTransaction) {
                    ...
                    // DELETE existing details and Serials to prevent duplication on update
                    try {
                        ...
                        if (existingDetailIds.length > 0) {
                            // 2. Delete existing Serials associated with these details
                            // First fetch the Serial IDs because bulk delete by filter might not be supported
                            const serialIdsRes = await fetcher(`/unfulfilled_sales_transaction_serial?filter[unfulfilled_sales_transaction_detail_id][_in]=${existingDetailIds.join(',')}&fields=id`);
                            const serialIds = (serialIdsRes.data || []).map((r: { id: number }) => r.id);

                            if (serialIds.length > 0) {
                                await fetch(`${BASE_URL}/unfulfilled_sales_transaction_serial`, {
                                    method: 'DELETE',
                                    headers: { 
                                        'Authorization': `Bearer ${TOKEN}`,
                                        'Content-Type': 'application/json'
                        ...
                        createdDetails.forEach((createdDetail: { id: number; sales_invoice_detail_id: number | { id: number } }) => {
                            ...
                            if (matchedTags && Array.isArray(matchedTags)) {
                                matchedTags.forEach((tag: string) => {
                                    ...
                                    if (validCheckedBy) {
                                        serialLog.created_by = validCheckedBy;
                                    }
                                    
                                    serialPayloads.push(serialLog);
                                });
                            }
                        });

                        if (serialPayloads.length > 0) {
                            try {
                                await poster<{ data: Record<string, unknown>[] | Record<string, unknown> }>('/unfulfilled_sales_transaction_serial', serialPayloads);
                            } catch (serialErr: unknown) {
                                const err = serialErr as Error;
                                console.error('Serial save error:', err?.message || err);
                            }
                        }
                    }
                }
