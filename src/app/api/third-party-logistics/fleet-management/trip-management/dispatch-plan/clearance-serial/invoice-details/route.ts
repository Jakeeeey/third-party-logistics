...
export async function GET(request: Request) {
    ...
    try {
        ...
        const [customerRes, productsRes, unitsRes, salesmanRes, branchRes] = await Promise.all([
            ...
            productIds.length > 0
                ? fetcher(`/products?filter[product_id][_in]=${productIds.join(',')}&fields=product_id,product_name,product_code,is_serialized&limit=-1`)
        ...
        const enrichedLines = lines.map((l) => {
            ...
            return {
                ...
                is_serialized: product?.is_serialized || 0
            };
        });
