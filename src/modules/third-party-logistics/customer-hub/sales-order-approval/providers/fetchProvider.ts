export async function getPendingOrders(status: string = "For Approval", search: string = "", page: number = 1, limit: number = 50, startDate: string = "", endDate: string = "") {
    ...
    const res = await fetch(`/api/third-party-logistics/customer-hub/sales-order-approval?${params.toString()}`);
...
export async function getOrderHeader(orderId: number) {
    const res = await fetch(`/api/third-party-logistics/customer-hub/sales-order-approval?type=order-header&orderId=${orderId}&_t=${Date.now()}`, { cache: "no-store" });
...
export async function getOrderDetails(orderId: number, branchId?: number | string) {
    let url = `/api/third-party-logistics/customer-hub/sales-order-approval?type=order-details&orderId=${orderId}`;
...
export async function getOrderAttachments(orderId: number, orderNo: string) {
    const res = await fetch(`/api/third-party-logistics/customer-hub/sales-order-approval?type=attachments&orderId=${orderId}&orderNo=${encodeURIComponent(orderNo)}`);
...
export async function getInvoiceDetails(orderId: number, orderNo: string) {
    const res = await fetch(`/api/third-party-logistics/customer-hub/sales-order-approval?type=invoice-details&orderId=${orderId}&orderNo=${encodeURIComponent(orderNo)}`);
...
export async function getPaymentSummary(orderIds: (string | number)[], orderNos: string[] = []) {
    ...
    let url = `/api/third-party-logistics/customer-hub/sales-order-approval?type=payment-summary&orderIds=${encodeURIComponent(idsStr)}`;
...
export async function updateOrders(
    ...
    const res = await fetch(`/api/third-party-logistics/customer-hub/sales-order-approval`, {
        method: "POST",
