        ...
        if (open && order) {
            const loadData = async () => {
                ...
                if (Object.keys(discountTypes).length === 0) {
                    promises.push(
                        fetch(`${window.location.origin}/api/third-party-logistics/customer-hub/sales-order-approval?type=discount-types`)
                        ...
                        <div className="px-4 sm:px-6 py-2 bg-muted/10 border-b border-border flex items-center gap-3 overflow-x-auto">
                            ...
                            <div className="flex items-center gap-2">
                                {attachments.map((att, i) => (
                                    <a
                                        key={i}
                                        href={`/api/third-party-logistics/customer-hub/callsheet/file?id=${att.file_id}&filename=${encodeURIComponent(att.attachment_name || "attachment.jpg")}`}
                                        target="_blank"
                                        rel="noreferrer"
