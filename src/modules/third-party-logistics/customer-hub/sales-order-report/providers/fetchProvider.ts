...
export const fetchSalesOrderData = async (
    ...
    const response = await fetch(`/api/third-party-logistics/customer-hub/sales-order-report?${queryParams.toString()}`);
    if (!response.ok) {
...
export const fetchSalesOrderDetails = async (orderId: number) => {
    const response = await fetch(`/api/third-party-logistics/customer-hub/sales-order-report?orderId=${orderId}`);
    if (!response.ok) {
...
export const fetchMonthlyAverage = async (customerCode: string) => {
    const response = await fetch(`/api/third-party-logistics/customer-hub/sales-order-report?type=mo-avg&customerCode=${customerCode}`);
    if (!response.ok) {
...
export const fetchInvoiceDetails = async (orderId: number, orderNo?: string) => {
    ...
    const response = await fetch(`/api/third-party-logistics/customer-hub/sales-order-report?${params.toString()}`);
    if (!response.ok) {
...
export const fetchOrderAttachments = async (orderNo: string) => {
    const response = await fetch(`/api/third-party-logistics/customer-hub/sales-order-report?type=order-attachments&orderNo=${orderNo}`);
    if (!response.ok) {
