...
export function useCustomerMap() {
  ...
  const fetchOptions = useCallback(async () => {
    try {
      ...
      const results = await Promise.all(
        fields.map(field => fetch(`/api/third-party-logistics/customer-hub/customer-map?field=${field}`).then(res => res.json()))
      );
  ...
  const fetchMapData = useCallback(async (currentFilters: CustomerMapFilter) => {
    ...
    try {
      ...
      const response = await fetch(`/api/third-party-logistics/customer-hub/customer-map?${params.toString()}`);
      if (!response.ok) {
