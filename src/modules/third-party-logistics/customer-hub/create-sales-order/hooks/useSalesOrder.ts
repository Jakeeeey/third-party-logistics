    ...
    useEffect(() => {
        const init = async () => {
            const [sm, sup, br, pt, rec, ops, pterms] = await Promise.all([
                ...
                fetch("/api/third-party-logistics/customer-hub/create-sales-order?action=invoice_types").then(r => r.json()),
                fetch("/api/third-party-logistics/customer-hub/create-sales-order?action=operations").then(r => r.json()),
                fetch("/api/third-party-logistics/customer-hub/create-sales-order?action=payment_terms").then(r => r.json())
            ]);
            ...
            if (currentId && (currentId !== lastId.current || !isAutoFilled.current)) {
                ...
                try {
                    ...
                    if (attachmentId) {
                        const attachment = await fetch(`/api/third-party-logistics/customer-hub/create-sales-order?action=get_attachment&id=${attachmentId}`).then(r => r.json());
                        ...
                        if (attachment) {
                            if (attachment.sales_order_id) {
                                ...
                                const lookup = await fetch(`/api/third-party-logistics/customer-hub/create-sales-order?action=get_order&order_no=${encodeURIComponent(attachment.sales_order_no)}`).then(r => r.json());
                    ...
                    if (finalSalesOrderId) {
                        ...
                        const orderData = await fetch(`/api/third-party-logistics/customer-hub/create-sales-order?action=get_order&order_id=${finalSalesOrderId}`).then(r => r.json());
    ...
    const handleSalesmanChange = async (id: string) => {
        ...
        if (id) {
            ...
            try {
                ...
                const res = await fetch(`/api/third-party-logistics/customer-hub/create-sales-order?action=accounts&user_id=${id}`);
            ...
            try {
                ...
                if (activeSalesmen.length === 1) {
                    ...
                    if (uid && (!selectedSalesmanId || !isCurrentValid)) {
                        ...
                        const res = await fetch(`/api/third-party-logistics/customer-hub/create-sales-order?action=accounts&user_id=${uid}`);
