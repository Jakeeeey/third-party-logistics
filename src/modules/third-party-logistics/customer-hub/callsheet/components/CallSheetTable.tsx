...
export function CallSheetTable({
    ...
    const handleViewDocument = (fileId: string, filename?: string) => {
        if (!fileId) return;
        const url = `/api/third-party-logistics/customer-hub/callsheet/file?id=${fileId}${filename ? `&filename=${encodeURIComponent(filename)}` : ""}`;
        window.open(url, "_blank");
    };

    const handleDownload = (fileId: string, filename?: string) => {
        if (!fileId) return;
        const url = `/api/third-party-logistics/customer-hub/callsheet/file?id=${fileId}&download=true${filename ? `&filename=${encodeURIComponent(filename)}` : ""}`;
        window.open(url, "_blank");
    };
