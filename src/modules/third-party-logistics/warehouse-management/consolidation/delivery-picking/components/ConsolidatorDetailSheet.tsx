...
export function ConsolidatorDetailSheet({consolidator, isOpen, onClose, onStatusUpdate}: ConsolidatorDetailSheetProps) {
    ...
    const fetchAndPrintManifest = async () => {
        setIsPrinting(true);
        try {
            const manifestRes = await fetch(`/api/third-party-logistics/warehouse-management/consolidation/delivery-picking/manifest/${consolidator.consolidatorNo}`);
            if (manifestRes.ok) {
    ...
    const handleConfirmPicking = async (checkerId: number) => {
        return new Promise<void>(async (resolve, reject) => {
            setIsSyncing(true);
            try {
                const res = await fetch(`/api/third-party-logistics/warehouse-management/consolidation/delivery-picking/start`, {
                    method: "POST",
