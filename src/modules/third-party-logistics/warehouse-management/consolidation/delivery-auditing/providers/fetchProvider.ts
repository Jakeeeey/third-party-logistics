...
export const fetchConsolidators = async (
    ...
    try {
        ...
        const url = `/api/third-party-logistics/warehouse-management/consolidation/delivery-picking?${params.toString()}`;
        const response = await fetch(url, { headers: getHeaders(), cache: "no-store" });
...
export const completeAuditBatch = async (batchId: number): Promise<boolean> => {
    try {
        const url = `/api/third-party-logistics/warehouse-management/consolidation/audit/complete`;
...
export const repickBatch = async (batchId: number): Promise<boolean> => {
    try {
        const url = `/api/third-party-logistics/warehouse-management/consolidation/audit/repick`;
        const response = await fetch(url, {
            method: "POST",
            headers: getHeaders(),
            body: JSON.stringify({ batchId }),
...
export const lookupRfidTag = async (rfidTag: string): Promise<number | null> => {
   try {
       const url = `/api/third-party-logistics/warehouse-management/consolidation/picking/lookup?rfid=${rfidTag}`;
       const response = await fetch(url);
       if (!response.ok) return null;
       const data = await response.json();
...
export const transmitAuditLog = async (payload: {
    ...
    try {
        const url = `/api/third-party-logistics/warehouse-management/consolidation/audit/log`;
        const response = await fetch(url, {
            method: "POST",
            headers: getHeaders(),
