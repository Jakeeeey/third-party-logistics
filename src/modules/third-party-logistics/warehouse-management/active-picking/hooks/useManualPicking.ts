...
import { transmitItemScan, submitManualPick } from "../providers/fetchProvider";
import { ConsolidatorDto, ConsolidatorDetailsDto } from "../types";
import { soundFX } from "../utils/audioProvider";
import { toast } from "sonner";
import { useRouter } from "next/navigation";

export interface UseManualPickingProps {
...
export function useManualPicking({ batch, currentUserId }: UseManualPickingProps) {
    const router = useRouter();

    // Shows ALL items — no RFID filter
    ...
    // 🚀 BARCODE SCANNER LOGIC — NO RFID tag lookup, direct barcode/productId match only
    const processScan = useCallback(async (inputString: string) => {
        ...
        try {
            ...
            // Direct barcode or productId match only — no RFID lookup
            const targetDetail = currentDetails.find(d =>
                d.barcode?.toLowerCase() === tag.toLowerCase() ||
                d.productId?.toString() === tag
            );

            if (!targetDetail) {
                logScan(tag, "error", "Unrecognized Barcode");
                return;
            }
            ...
            // Transmit with empty rfidTag (barcode/manual mode)
    ...
    // 🚀 HARDWARE LISTENER (barcode scanner)
    useEffect(() => {
