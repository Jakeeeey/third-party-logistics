import type {
  ...
  FuelTypeApiRow,
  EngineTypeApiRow,
} from "@/modules/third-party-logistics/fleet-management/vehicle-management/vehicle-list/types";
...
export async function listVehicleTypes(): Promise<VehicleTypeApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/vehicle-type");
...
export async function listFuelTypes(): Promise<FuelTypeApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/fuel-type?limit=-1");
  if (!res.ok) throw new Error(await readError(res));
...
export async function listEngineTypes(): Promise<EngineTypeApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/engine-type?limit=-1");
  if (!res.ok) throw new Error(await readError(res));
...
export async function listVehicles(): Promise<VehiclesApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list");
  if (!res.ok) throw new Error(await readError(res));
...
export async function listUsers(): Promise<UserApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/users?limit=-1");
  if (!res.ok) throw new Error(await readError(res));
...
export async function listDispatchPlans(): Promise<DispatchPlanApiRow[]> {
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/dispatch-plans?limit=-1");
  if (!res.ok) throw new Error(await readError(res));
...
export async function listDispatchPlansByVehicle(vehicleId: number): Promise<DispatchPlanApiRow[]> {
  const res = await fetch(
    `/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/dispatch-plans?limit=-1&vehicle_id=${encodeURIComponent(
      String(vehicleId)
    )}`
  );
  if (!res.ok) throw new Error(await readError(res));
...
export async function uploadVehicleImage(file: File): Promise<string> {
  ...
  const res = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list/upload", {
    method: "POST",
    body: fd,
  });
...
export async function createVehicle(payload: Record<string, unknown>) {
  const first = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list", {
    method: "POST",
  ...
  const second = await fetch("/api/third-party-logistics/fleet-management/vehicle-management/vehicle-list", {
    method: "POST",
    headers: { "content-type": "application/json" },
