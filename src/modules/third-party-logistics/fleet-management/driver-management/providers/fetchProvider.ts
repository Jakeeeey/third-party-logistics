...
export async function fetchDriversWithDetails(): Promise<{
    ...
    const res = await fetch("/api/third-party-logistics/fleet-management/driver-management");
...
export async function deleteDriver(id: number): Promise<unknown> {
    const res = await fetch(`/api/third-party-logistics/fleet-management/driver-management?id=${id}`, {
        method: "DELETE",
    });
    if (!res.ok) {
        const err = await res.json();
