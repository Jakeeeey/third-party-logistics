import {
  ...
  UserRow,
  VehicleRow,
} from "../types/for-dispatch-summary.types";
 ...
 */
export async function fetchForDispatchSummaryData() {
  if (!DIRECTUS_BASE) {
  ...
  const [
    ...
    fetchDirectus<PostDispatchPlanRow[]>(
      `/items/post_dispatch_plan?limit=-1&fields=id,doc_no,driver_id,vehicle_id,status,estimated_time_of_dispatch,estimated_time_of_arrival,time_of_dispatch,remarks&filter[status][_eq]=For Dispatch&sort=estimated_time_of_dispatch`,
    ),
 ...
 */
export async function fetchForDispatchSummaryClient() {
  const res = await fetch(
    `/api/third-party-logistics/fleet-management/trip-management/for-dispatch-summary`,
    { cache: "no-store" },
  );

  if (!res.ok) {
    const txt = await res.text().catch(() => "");
    throw new Error(txt || "Failed to fetch for-dispatch summary");
  }
