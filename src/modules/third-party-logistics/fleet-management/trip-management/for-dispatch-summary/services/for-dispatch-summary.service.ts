import { fetchForDispatchSummaryData } from "./for-dispatch-summary.repo";
import { normalizeCode } from "./for-dispatch-summary.helpers";
import type {
  ForDispatchInvoice,
  SalesInvoiceRow,
  CustomerRow,
...
export async function getEnrichedForDispatchSummary(): Promise<ForDispatchInvoice[]> {
  const {
    rawPlans,
    staff,
    dispatchInvoices,
    ...
    users,
    vehicles,
  } = await fetchForDispatchSummaryData();
  ...
  const enrichedInvoices: ForDispatchInvoice[] = [];
