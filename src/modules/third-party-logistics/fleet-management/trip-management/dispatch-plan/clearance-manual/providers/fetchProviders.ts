...
export const getJoinedDispatchData = async (
  ...
  const response = await fetch(`/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/clearance?${query}`);
  if (!response.ok) {
    throw new Error(`HTTP error! status: ${response.status}`);
  }
  return response.json();
...
export const submitClearance = async (dispatchId: number, invoices: ReconciliationRow[], isPreSave: boolean = false): Promise<void> => {
  const response = await fetch('/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/clearance', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export const fetchInvoiceDetails = async (invoiceId: number): Promise<InvoiceDetail> => {
  const response = await fetch(`/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/clearance/invoice-details?invoice_id=${invoiceId}`);
  if (!response.ok) {
    throw new Error(`HTTP error! status: ${response.status}`);
  }
  return response.json();
...
export const fetchRFIDTagsForDispatch = async (dispatchId: number): Promise<RFIDMapping[]> => {
  const response = await fetch(`/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/clearance/rfid-tags?dispatch_id=${dispatchId}`);
  if (!response.ok) {
    throw new Error(`HTTP error! status: ${response.status}`);
  }
  return response.json();
...
export const fetchSalesReturnsByInvoice = async (invoiceNo: string): Promise<{ id: number; returnNo: string; returnDate: string; totalAmount: number }[]> => {
  ...
  const response = await fetch(`/api/third-party-logistics/inventories/sales-return-manual?action=list&invoiceNo=${encodeURIComponent(invoiceNo)}`);
  if (!response.ok) {
    throw new Error(`HTTP error! status: ${response.status}`);
  }
