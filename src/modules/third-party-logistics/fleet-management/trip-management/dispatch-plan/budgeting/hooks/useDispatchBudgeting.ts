...
export function useDispatchBudgeting() {
  ...
  const fetchSummary = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/budgeting/summary",
        { cache: "no-store" },
      );
      const result = await res.json();
      if (result.error) throw new Error(result.error);
  ...
  const updateBudget = async (
    ...
    try {
      const res = await fetch(
        `/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/budgeting?plan_id=${planId}`,
        {
          method: "PATCH",
  ...
  const fetchPlanBudgets = async (planId: number) => {
    const res = await fetch(
      `/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/creation?type=plan_budgets&plan_id=${planId}`
    );
    const result = await res.json();
    if (result.error) throw new Error(result.error);
    return result.data || [];
