...
import { DispatchRow, ReconciliationRow, SerialMapping } from '../types';
import { submitClearance, fetchSerialNumbersForDispatch } from '../providers/fetchProviders';
...
const ClearanceModal: React.FC<ClearanceModalProps> = ({ isOpen, onClose, onSuccess, dispatch }) => {
    ...
    const [serialNumbers, setSerialNumbers] = useState<SerialMapping[]>([]);
    const [invoices, setInvoices] = useState<ReconciliationRow[]>([]);

    // Fetch serial numbers when modal opens — intentional data fetch pattern
    useEffect(() => {
        if (isOpen && dispatch.id) {
            fetchSerialNumbersForDispatch(dispatch.id)
                .then(setSerialNumbers)
                .catch(err => console.error("Failed to fetch serial numbers:", err));
        }
    }, [isOpen, dispatch.id]);
                ...
                <div className="p-4 md:p-6 space-y-6 overflow-y-auto flex-1 custom-scrollbar bg-background">
                    ...
                    <Card className="border-border shadow-sm overflow-hidden rounded-xl bg-card">
                        <CardHeader className="py-4 px-6 bg-muted/30 border-b border-border">
                            <div>
                                <h3 className="text-sm font-bold text-foreground">Invoice Reconciliation Table (Serial)</h3>
