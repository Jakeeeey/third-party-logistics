...
import { InvoiceLine, SerialMapping } from '../types';
import { Input } from '@/components/ui/input';

interface ScanningModalProps {
    isOpen: boolean;
    onClose: () => void;
    onConfirm: (scannedQtys: Record<string | number, number>, scannedSerials: Record<string | number, string[]>) => void;
    items: InvoiceLine[];
...
const ScanningModal: React.FC<ScanningModalProps> = ({
    ...
    const scannedSerialsRef = useRef<Record<string | number, string[]>>(initialScannedSerials);
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            scannedSerialsRef.current = initialScannedSerials;
    ...
    const handleSerialScan = (input: string) => {
        ...
        tags.forEach(tag => {
            ...
            const previousTags = scannedSerialsRef.current[item.id] || [];
    ...
    const handleManualQtyChange = (itemId: string | number, delta: number) => {
        const item = items.find(i => String(i.id) === String(itemId));
        if (!item || item.is_serialized === 1) return;

        const currentQty = scannedQtysRef.current[itemId] || 0;
        const newQty = Math.max(0, Math.min(item.qty, currentQty + delta));
        
        scannedQtysRef.current = { ...scannedQtysRef.current, [itemId]: newQty };
        setScannedQtys(scannedQtysRef.current);
    };
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (e.key === 'Enter') {
                if (bufferRef.current) {
                    handleSerialScan(bufferRef.current);
                    bufferRef.current = '';
                }
    ...
    }, [isOpen, isScanning, serialNumbers, items, scannedQtys, scannedTags]);
    ...
    const handleConfirm = () => {
        ...
        onConfirm(finalScanned, scannedSerialsRef.current);
        onClose();
    };
    ...
    const uniqueUnits = [...new Set(items.map(i => i.unit))];
    const unitLabel = uniqueUnits.length === 1 ? uniqueUnits[0] : 'Units';
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            ...
            >
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 overflow-hidden">
                            ...
                            <div className="space-y-0.5 truncate">
                                <DialogTitle className="text-lg md:text-xl font-bold truncate text-foreground">Serial Scanning</DialogTitle>
                                <p className="text-[10px] md:text-xs text-muted-foreground font-medium truncate">
                                    {isScanning ? 'Waiting for Serial number detection...' : 'All items accounted for.'}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 pt-2 space-y-6 custom-scrollbar bg-background">
                        ...
                        <div className="relative z-10 flex items-center justify-between mb-4">
                            <div className="space-y-1">
                                ...
                                <h3 className="text-2xl md:text-3xl font-black">
                                    {totalScanned} <span className="text-sm md:text-lg font-medium text-primary-foreground/70">/ {totalRequired} {unitLabel}</span>
                                </h3>
                            </div>
                    ...
                    {/* Manual Serial Entry */}
                    <div className="p-4 rounded-xl border border-border bg-muted/30 space-y-3 shrink-0">
                        <div className="flex items-center gap-2">
                            <div className="p-1.5 rounded-lg bg-orange-500/10 text-orange-500">
                                <Keyboard className="w-4 h-4" />
                            </div>
                            <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Manual Serial Entry</p>
                        </div>
                        <div className="flex gap-2">
                            <Input
                                ref={inputRef}
                                placeholder="Type or scan serial number here..."
                                value={manualInput}
                                onChange={(e) => setManualInput(e.target.value)}
                                onKeyDown={(e) => {
                                    if (e.key === 'Enter') {
                                        handleSerialScan(manualInput);
                                        setManualInput('');
                                    }
                                }}
                                className="h-11 rounded-xl border-border bg-background focus:ring-2 focus:ring-primary/20 text-sm font-medium"
                                autoFocus
                            />
                            <Button 
                                onClick={() => {
                        ...
                        <p className="text-[10px] text-muted-foreground italic">
                            Press <strong>Enter</strong> to quickly add the serial.
                        </p>
                    </div>
                    ...
                    {lastScanned && (
                        <div className="p-3 md:p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/10 flex items-center justify-between animate-in fade-in slide-in-from-top-2 shrink-0">
                            <div className="flex items-center gap-2 md:gap-3 overflow-hidden">
                                ...
                                <div className="space-y-0.5 truncate">
                                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-wider">Detected Serial</p>
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            {item.is_serialized !== 1 && (
                                                                <Button
                                                                    variant="outline"
                                                                    size="icon"
                                                                    className="h-7 w-7 rounded-full border-border bg-background hover:bg-muted"
                                                                    onClick={() => handleManualQtyChange(item.id, -1)}
                                                                    disabled={scanned === 0}
                                                                >
