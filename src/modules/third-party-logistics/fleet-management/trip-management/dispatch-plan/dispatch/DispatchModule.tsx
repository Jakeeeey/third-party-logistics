...
export default function DispatchModule() {
    const { filteredPlans, users, loading, search, setSearch, reload } = useDispatch();
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                    <div className="space-y-3">
                        ...
                        <h1 className="text-5xl font-black tracking-tighter text-foreground leading-[0.9]">
                            Dispatch <span className="text-emerald-600">Manual</span>
                        </h1>
                        <p className="text-lg text-muted-foreground font-medium max-w-xl">
                            Verify and confirm trip dispatch manually. Review personnel assignments and record dispatch timestamps in real-time.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <div className="flex flex-col items-center justify-center bg-card/40 border border-border/40 shadow-xl rounded-[2rem] px-8 py-6 min-w-[120px] backdrop-blur-md transition-all hover:border-emerald-500/30 group">
                            ...
                            <div className="flex flex-col items-center leading-none mt-2 opacity-40">
                                <span className="text-[10px] text-foreground uppercase font-black tracking-widest">Active</span>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4">
                <div className="relative flex-1 group">
                    ...
                    <Input 
                        ...
                        className="pl-14 h-16 rounded-[1.25rem] border-border/40 bg-card/40 backdrop-blur-sm text-lg font-bold transition-all focus:ring-emerald-500/20 focus:border-emerald-500/40"
                        value={search}
                ...
                <Button 
                    variant="outline" 
                    size="icon" 
                    className="h-16 w-16 rounded-[1.25rem] border-border/40 bg-card/40 backdrop-blur-sm hover:bg-emerald-500/5 hover:border-emerald-500/30 text-muted-foreground transition-all active:scale-95"
                    onClick={() => reload()}
                    disabled={loading}
                >
                    <RefreshCcw className={`h-6 w-6 ${loading ? 'animate-spin text-emerald-600' : ''}`} />
                </Button>
            </div>
