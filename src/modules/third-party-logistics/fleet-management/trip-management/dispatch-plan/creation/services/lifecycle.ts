...
import type { PostDispatchBudgetRow } from "@/modules/third-party-logistics/fleet-management/trip-management/dispatch-plan/creation/types/dispatch.types";
...
export const dispatchCreationLifecycleService = {
  ...
  async updateTrip(planId: number, payload: DispatchCreationFormValues) {
    const response = await fetch(
      `/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/creation?plan_id=${planId}&action=update_trip`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  async updateBudget(planId: number, budgets: Omit<PostDispatchBudgetRow, "id">[]) {
    const response = await fetch(
      `/api/third-party-logistics/fleet-management/trip-management/dispatch-plan/creation?plan_id=${planId}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
