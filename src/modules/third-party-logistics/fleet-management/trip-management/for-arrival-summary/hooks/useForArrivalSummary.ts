...
import { fetchForArrivalSummaryClient } from "../services/for-arrival-summary.repo";
import { buildDispatchPlanGroup, matchesSearch } from "../services/for-arrival-summary.helpers";

export function useForArrivalSummary() {
  const [invoices, setInvoices] = React.useState<ForArrivalInvoice[]>([]);
  ...
  const reload = React.useCallback(async () => {
    ...
    try {
      const data = await fetchForArrivalSummaryClient();
      setInvoices(data);
    } catch (e) {
      setInvoices([]);
  ...
  const dispatchPlanGroups = React.useMemo<DispatchPlanGroup[]>(() => {
    const grouped = new Map<string, ForArrivalInvoice[]>();
